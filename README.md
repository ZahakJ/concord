...
**A serverless, end-to-end-encrypted Discord alternative — your community, on
your machines, readable by no one else.**
...
cryptographic group; every message is end-to-end encrypted; the one piece of
infrastructure it can use is untrusted by construction.

This document is a design paper. It explains what peer-to-peer actually means,
defines every term it uses the first time it appears, and describes each
mechanism precisely enough that you could argue with it. It is written to be
read straight through without opening the source; a [map of the
code](#17-map-of-the-code) is at the end for when you want to. For the short,
...
14. [Running Concord](#14-running-concord)
15. [Playing with friends over the internet](#15-playing-with-friends-over-the-internet)
16. [Feature status](#16-feature-status)
...
fix: **the operator sits between every message you send and every friend who
reads it.** The operator authenticates you, stores your history, relays your
voice, and — with the best intentions in the world — *can* read, mine, moderate,
lose, leak, or be compelled to hand over all of it. Transport encryption (TLS)
does not change this: the server terminates TLS, so the plaintext exists on the
operator's machines by design. Every property of your community — availability,
privacy, history, membership — is a promise, not a guarantee you hold.

Concord's thesis, in one sentence:

> **If the participants do the work that a server would otherwise do, then
> nobody has to be trusted — and the price for that is that a room can only be
> as big as its members' own bandwidth.**

Everything below is either a mechanism that discharges some server's job onto
the participants, or an honest statement of what that costs.

### What Concord refuses to have
...
- **No second product.** If it isn't in the binary, it isn't in Concord. This
  line was learned the hard way: an optional, local, off-by-default helper that
  shelled out to a separately installed program was written, removed, written
  again, and removed again before the rule was stated correctly. "Optional" and
  "local" did not stop it being a thing the user has to install, a second
  program to keep working, and a paragraph in the docs explaining why the
  feature is missing on their machine. A dependency you can decline is still a
  dependency. Whatever ships must work on a fresh machine with nothing else
  installed.

### The deliberate trade

Concord targets **friend groups and communities**, not million-user servers.
Text scales fine; a full mesh of live audio connections does not scale to
stadiums, and does not need to. That single trade is what buys everything else,
and it is stated up front rather than discovered later.
...
Discovery and reachability are frequently confused, so it is worth being blunt:
**discovery is finding out where someone is; reachability is being able to send
them a packet once you know.** They are solved by different mechanisms, they
fail independently, and each has its own set of fallbacks. §4 and §5 are kept
...
your friend a sealed, tamper-evident envelope that only they can open — and
...
it, rename it, or issue a second one — and nobody can help you if you lose it,
...
it in the PeerID verbatim rather than hashing it — so **a PeerID is a public key
in disguise.**
...
### Fingerprints, and what verification actually proves

A PeerID is 52 characters of base58 — fine for a machine, useless for reading
...
This is what Signal calls a *safety number*. Its purpose is narrow and worth
stating exactly:
...
(§3, *devices*, below). That notice is the half of safety numbers that catches an
...
- Every authorization decision normalizes a credential to an account first: a
  ...
  verifies*, otherwise the raw bytes are used, so a forged certificate cannot
  masquerade as somebody else's account — it just becomes an unknown identity.
...
that it certifies *its own* device key, and that the seed it was given actually
derives the account in the certificate, before writing anything to disk.
...
fresh 16-byte salt per save. Argon2id is memory-hard: guessing your passphrase
costs an attacker 64 MiB of RAM per guess, which is what makes offline
brute-force expensive rather than free. The parameters are recorded in the file
so they can be raised later without breaking existing keystores. The plaintext
seed never touches disk. A wrong passphrase is indistinguishable from a
corrupted file, deliberately.

Three further keys are derived from the seed by HKDF-SHA256 with distinct labels
— `concord-store-v1` for the local database, `concord-mls-sig-v1` for the group
signing key, `concord-mbx-enc-v1` for the offline mailbox. Distinct labels mean
that even a total break of one context cannot be levered into another, and
because they are *derived* rather than stored, a restarted device recomputes them
and carries on with nothing to back up.
...
which machine holds it?* — answered without asking anyone, because asking
requires a directory, which is what we are building.
...
XOR distance looks strange until you notice what it measures: **the length of
the shared prefix.**
...
distance there is exactly one identifier at that distance, so lookups from
...
holds up to *k* peers whose IDs share exactly *i* leading bits with its own. In
Concord's DHT *k* = 20. So a node knows a great many peers "near" itself and
only a handful in each successively more distant half of the space — a routing
table of about log₂(n) buckets, not n entries.
...
comes from a node with a longer shared prefix with K than the last, which means
**each round fixes at least one more bit** and halves the remaining search
space:
...
this DHT) and are simply forgotten when it lapses, so the publisher republishes
— every 3 hours, in Concord's case. Nothing has to be deleted when a node dies;
its records age out on their own. A DHT is therefore a *self-cleaning*
directory, which matters a great deal when the things being indexed are laptops.
...
so. That is a *rendezvous point* — an agreed-upon location where parties who
have never met can each leave a note for the other.
...
- **A DHT is only distributed if it has many servers.** A node that cannot
  ...
  Kademlia machinery is what makes that degrade gracefully into a genuinely
  distributed table the moment there is more than one server — a second
  rendezvous, a friend on a public IP, or the public network in rung 4.

### 4.6 The ladder Concord actually climbs
...
**Rung 2 — remembered peers.** Every peer you actually shared a guild with is
written to a small local cache with its addresses and a last-seen timestamp.
This is the rung that needs *nothing*: if the rendezvous is gone, yesterday's
friends are still dialable at yesterday's addresses. Specifics, because "we
remember peers" is the kind of claim that hides a lot:

- at most 64 peers, at most 8 addresses each;
- forgotten after 30 days without a successful connection;
- retired after 5 consecutive *outages* — counted at most once per outage, never
  once per dial attempt, so the friend who happens to be asleep on the night the
  rendezvous dies is not deleted by a retry loop;
- addresses are only recorded for peers you share a guild with, and only
  addresses at a host that traffic genuinely arrived from (a peer can *claim* any
  address in the handshake; a claim is not evidence);
- dialled on the first pass of every launch — a restart should reach yesterday's
  friends before any DHT lookup completes — and after that only while the
  rendezvous is unreachable.

Honest gap: this cache is **plaintext on disk**. See §13.
...
price is precisely the metadata described in §4.5, now paid to strangers: your
peer ID and addresses become visible on a public network, and the key you
advertise under is guessable, so an observer can enumerate Concord nodes.
Messages stay sealed; the fact of running Concord does not. That is the user's
trade to make, so it is off unless they make it.

### 4.7 Invite codes: the out-of-band rung

There is a fifth path, and it is the one most people actually use first. An
**invite code** is a compact base64 blob you paste into a chat, carrying the
guild's opaque handle and name, the inviter's peer ID, the inviter's current
addresses, and the rendezvous list the inviter is using. It is discovery by
human courier: the joiner's app configures itself entirely from the code, dials
the addresses it names, and asks to join.

Two design details worth naming. Relay addresses are only included once a relay
reservation is genuinely held, because libp2p only lists a circuit address when
it will actually work, and a code is a permanent artifact — a confidently wrong
address pasted into a chat costs every future joiner a dial timeout. And a
rendezvous the inviter was *configured* with is advertised in its DNS form rather
than the IP snapshot libp2p renders, because a DNS name survives the host being
...
### 5.1 What NAT is, and exactly why it breaks inbound connections
...
The critical detail is that **the table entry is created by an outbound packet.**
An inbound packet that matches no entry names no internal destination the router
...
libp2p's answer is **DCUtR** — *Direct Connection Upgrade through Relay*. The
two peers first establish a **relayed** connection (§5.3), then use that channel
to exchange their observed addresses and to measure the round-trip time so both
can start dialling at the same instant. The relay is scaffolding: it exists to
carry the handshake that makes it unnecessary.

**Success rate, honestly.** This is a property of the internet's NAT population,
not of Concord. libp2p's own measurement campaigns report roughly 70% success
over TCP and roughly 80% over QUIC. In practice most pairs go direct; a minority
— symmetric NATs, some carrier-grade NAT deployments, restrictive corporate
firewalls — never do, and for those the relay is not a stepping stone but the
connection.
...
setting that buys a genuinely unmediated connection. It exists as a setting
because it needs the port to be *stable*: an ephemeral port changes every launch
and no forwarding rule can follow it. Concord checks the port is genuinely free
before binding (libp2p will not reliably tell you — its TCP transport sets
...
- So a relay learns exactly: Alice's IP, Bob's IP, that they are connected, when,
  and how many bytes. Nothing else is available to it, not as a matter of policy
  but of construction.
...
- **Rendezvous nodes are offered first** — dedicated, always public, and named
  in invite codes.
...
- **Only vouched-for peers are offered as our relay.** Whoever relays for you
  ...
  addresses, when they connect — and the circuit address ends up in your invite
...
Honest gap: relay privileges, once granted, are never withdrawn. See §13.
...
complete answer: what it runs, what it can see, what it cannot, and precisely
what stops working the day it dies.
...
Its properties are worth spelling out because each one is a deliberate refusal:
...
- The envelope is sealed with an **ephemeral sender key**, so the node cannot
  learn who deposited it even in principle — only which peer's connection
  carried the deposit, which it needs for fair accounting.
...
mailbox per 30 seconds. This is the **only** component that stops being
commodity infrastructure, because it holds Apple/Google credentials. With no
...
- **The outbound request carries headers the node chose** — one constant
  user-agent, no cookies, no referrer, nothing forwarded from the peer — so the
...
the reasons are worth separating:
...
exactly this, and now shipping in Apple and Google's messengers. In Concord
**every guild is one MLS group**, and:
...
- An **epoch** is a version number for the group. Each epoch has its own key
  ...
  derived from epoch *e−1*'s plus fresh entropy — forward only, through a
...
- A **commit** is the message that moves the group from epoch *e* to epoch
  *e+1*. It carries the tree updates, is signed by the committing member's leaf
  key, and must be applied by every other member — **gaplessly and in order**,
  which is what makes comparing two members' epoch numbers a precise statement
  of exactly which commits one of them is missing. That property is the basis of
  history sync's commit backfill (§11).
...
> **The honest limit, stated plainly:** forward secrecy protects *captured
> network traffic*. It does not protect *your local database*, which holds your
...
> **The honest limit:** MLS delivers post-compromise security when a member
> performs an update. Concord's only commits are adds and removes, so a guild
> with a stable roster does not rekey on its own. Post-compromise security is
> realised at the next membership change, not on a timer. Discord and most chat
> applications have neither property at all, but "we rekey on membership change"
> is a smaller claim than "we rekey continuously", and it is the true one.
...
would be the exact hand-waving this document is trying to avoid.
...
log slot) but concurrency across two machines is genuinely unhandled, and the
recovery path is re-admission rather than merge.
...
One honest exposure. Commits are MLS *public* messages — signed, not encrypted —
because a receiver must be able to identify the committer before applying
anything. The control topic's name is a hash of the group identifier, so only
members can derive it. But a **removed** member still knows that identifier, and
can therefore keep subscribing and watch the roster change after their removal.
Message content stays sealed; membership churn does not.
...
- **Edits, deletes, reactions and pins ride the same channel.** They are
  ...
  encrypted, equally authenticated. Each re-emits the updated state so every
...
the browser already has a well-tested stack for exactly that. So Concord splits
the job: **the browser does the media, Go does the matchmaking.**
...
authenticated channel at all; it does not need a special one. Concord carries it
over a dedicated libp2p stream between the two peers, one fire-and-forget framed
...
signalling message so a page refresh is distinguishable from a heartbeat — needed
because a relayed connection's allocation can outlive the tab and keep reporting
itself healthy for minutes. A mesh is why voice targets small rooms: it is the
honest bandwidth ceiling of pure peer-to-peer.
...
### 9.2 IP privacy: the honest state of it
...
unremarkable — it is equally true of BitTorrent and most VoIP. For a **meeting
link you paste somewhere public** it is a deanonymization vector: a stranger who
...
client and neither participant sees the other's address. The relay does — it has
to, in order to forward — but the relay is already untrusted infrastructure that
cannot read the media, which stays DTLS-SRTP encrypted end to end. The trade is
moving one piece of metadata from "any stranger with a link" to "the one node you
already bootstrap through". Credentials are time-windowed HMACs derived from a
shared secret, so the secret itself never ships to a client.

Concord implements this. It is **off on the reference deployment, deliberately**,
and the reason is measured rather than theoretical:
...
"hide my IP". Those calls then had exactly one candidate and it was useless.
Chat kept working — chat is libp2p, not WebRTC — while audio never arrived and
shared screens stayed black.

So the relay now **refuses to start** unless it can produce a genuinely routable
relay address, resolving the deployment's own public hostname when it has not
been told its IP explicitly. The trade is stated in the code and worth stating
here:

> **A dead relay is worse than no relay.** No relay means calls connect
> directly: you lose IP privacy, which is a real loss, but a working call beats a
> private one that cannot carry sound.

When no relay is available, the app says so — "No relay available, this call
won't hide your IP" — greys out the "hide my IP" switch, and connects directly. A
guest page with no relay prints the same warning in plain words. Nothing silently
pretends.
...
One more honest note about outbound contact during a call. When no rendezvous
supplies ICE configuration, the fallback is a public Google STUN server, which is
contacted at call time to learn your own external address. It fetches no assets
and reveals nothing but your IP and the fact that you are starting a call — but
it is a third party, it is hardcoded, and this document would be dishonest not to
name it.
...
client, a contractor, somebody who will be in exactly one meeting. Guests are
that path, and they are the one place Concord relaxes its trust model — so the
relaxation is bounded on every axis.
...
it cannot, it has never seen one — and it does not read the meeting.
...
hours all week", not a duration puzzle, and a fixed set is also what stops a peer
talking your client into keeping a room forever.
...
reading it should understand it exactly as the UI presents it: *somebody who
called themselves this, relayed by the host.*

Guest media is ordinary WebRTC, direct between browser and member app, DTLS-SRTP
encrypted, never through the rendezvous. Guest meetings are precisely the case
that most wants the TURN relay of §9.2 — and, with no relay available, the guest
page says so in plain words rather than quietly exposing everyone.
...
- **What stays in the clear** is worth reading twice, because "encrypted at
  ...
  social life — who, where, when, in which rooms — while message content stays
  sealed. §13 states this as a gap rather than leaving you to infer it.
...
structurally cannot offer you that. Unread counts are deliberately computed with
SQL alone, decrypting nothing. **Export** dumps a channel to Markdown, because
your data is yours.
                          ...
                          "for guild G: I am at epoch 7, and here is my
                           latest timestamp per channel"
                                       ...
                                       ① MLS commits after epoch 7, in the
                                          ...
                                          on arrival exactly as if they had
                                          come live)
...
serving member* — their local copies — not re-verified against each original
sender. This is the same trust a centralized app places in its server, narrowed
to people who are already in your guild. Two real gates sit on top of it: MLS
commits are re-authorised through the §7.4 governance gate as the epoch advances,
so a member cannot smuggle in an unauthorised membership change; and
*destructive* reconciliation — tombstoning, overwriting, replacing reactions,
overwriting an already-known profile — is restricted to the guild owner or a
member holding the sync-host permission. An ordinary member may only fill gaps.
The residual gap is that an ordinary member could forge a *new* message
attributed to somebody else; closing it wants per-message author signatures.
...
Fetching walks connected members of the relevant guild one at a time (not fanned
out), verifies the hash before storing, and moves on if it does not match.
Serving is deliberately ungated — any peer that asks gets the ciphertext, because
it is meaningless without the key and the 256-bit identifier is unguessable. The
payoff is that **every member who has viewed an image becomes a source**, which
is what keeps pictures loadable after the sender goes offline. Caps: 5 MiB per
...
templates**, and **eight typefaces** in 22 subsetted WOFF2 files. That is
roughly 25 MB of the binary, and it is deliberate.

**The principle: no webfont, icon or image is ever fetched at runtime.** A
request to a font host or an image CDN on every launch tells that host your IP
address and the moment you opened the application. Repeat it daily and it is an
activity log kept by a third party you never chose, describing an app whose
entire purpose is that no third party knows anything. There is no way to have
this feature and the CDN. So the assets ship in the binary, every stylesheet
reference is same-origin, and the favicon is an inline data URI rather than one
more request.
...
- **Link previews and YouTube embeds** are **off by default**, and when off the
  ...
  Google"). Turning them on is one click; so is a second click to actually load
  an embed. Non-YouTube previews are fetched by the local backend behind an SSRF
  guard, not by the browser.

### An honest asymmetry in how packs are received

Worth documenting because it is a real hole, self-documented in the source, and
the kind of thing that should be in a design paper rather than discovered later.
...
What bounds the damage: records are validated exactly as a local addition would
be (name and tag patterns, a 500-entry pack ceiling for new identifiers, image
data URIs restricted to PNG/JPEG/GIF/WebP under 256 KiB — SVG excluded, so no
...
signature, the way governance operations already do (§7.4) — a larger change than
moving one line, and one that should cover both at once. Until then it is stated
...
| A stranger learning anything by messaging you | An unaccepted message request discloses nothing — not your profile, not your mailbox key, not that you are online (§16). |
...
| Not protected | The honest detail |
|---|---|
...
| **`peers.json` is a plaintext contact list** | Remembering peers so friendships survive the rendezvous means writing peer IDs, addresses and last-seen times beside the encrypted store, readable without the passphrase. Each entry converts directly into the fingerprint the app shows for verification, so a stolen laptop or a config directory swept into a cloud backup yields the social graph — who, where, when — while every message stays sealed. Individually these addresses crossed the wire anyway; the *set* is what the encrypted store is otherwise careful never to write in the clear. It wants sealing with the same key as everything else, which means loading it after unlock rather than before, which is why it is not done yet. |
...
| **What it costs you** | — | **Small rooms; metadata is not hidden** |
...
passphrase, then create or join guilds, chat, manage members, and verify
contacts.
...
**A local rendezvous**, to exercise internet-style discovery without deploying:

```sh
make rendezvous-run   # stable identity; point peers at the printed multiaddr
```

### Tests

```sh
make test           # full suite (incl. a multi-node E2EE integration test)
...
parses newline-terminated JSON lines — and each half once looked correct alone
while the pair was broken for weeks (the gateway forwarded lines without the
trailing `\n`; every guest join hung at "Connecting…"). Two guards now cover it:

- *Automatic* — `cmd/rendezvous/guest_smoke_test.go` runs in the normal
  `go test ./...` suite. It drives the real gateway handler over a real libp2p
  stream and a real WebSocket, and parses the gateway's output with a literal Go
  transcription of the guest page's `ws.onmessage` buffer-and-split loop — so the
  test *is* the other half of the framing contract, and any frame the page
  couldn't parse fails the suite.
- *On demand* — `scripts/smoke-guest.sh` is the full browser end-to-end: it
  builds and launches a local rendezvous + member app, mints a real guest link,
  and drives member and guest in two Chromium contexts, asserting chat both ways
  plus WebRTC audio packets and screen-share frames flowing in both directions.
  Needs `chromium` and a `playwright-core` install (`PLAYWRIGHT_CORE=<path>`);
  see the script header.

---

## 15. Playing with friends over the internet
...
1. Download one file (the release binary for their OS) and run it — the browser
   opens by itself.
...
rendezvous** — the friend's app configures itself from the code alone (§4.7).
...
**On the TURN relay: do not enable it on fly.io.** The deployment file ships with
that service commented out and the measurement recorded next to it. See §9.2 for
why, and for what to do instead if IP-private calls matter to you: put the relay
on a host that owns its public IP and point `CONCORD_TURN_*` there. Re-enabling
it on fly without re-testing end-to-end media will resurrect a silent failure in
which calls connect, report success, and carry no sound.
...
- **Desktop apps** — `concord-desktop-linux` (bare binary),
  ...
  bundle zipped). Wails needs each OS's native WebView + cgo, so the non-Linux
  desktop builds need that OS (or CI).
- `SHA256SUMS` (hashes of every asset), its signature, and `WINDOWS.md`.
...
**Update trust is a signature, not a host.** `SHA256SUMS` lists every asset's
hash, so signing that one file covers them all. Releases are signed with an
offline Ed25519 key whose public half is compiled into the binary; verification
checks the signature over `SHA256SUMS` *first*, then checks the download against
the now-trusted hash — never the other way round. Because a signature is what is
trusted, **where** the bytes came from stops mattering, which is what makes
peer-to-peer updates safe: a peer already running the newer build can hand you
the exact same bytes, and it is a courier rather than an authority. The worst it
can do is refuse, which is why GitHub remains a source rather than being
replaced — a peer who withholds an update must not be able to silence it. A build
with no embedded key refuses peer updates outright.
...
SmartScreen reputation on every new build. After a release, submit
`concord-desktop-windows.exe` to
<https://www.microsoft.com/en-us/wdsi/filesubmission> (category: incorrectly
detected / false positive) to whitelist that build's hash in Defender. This must
be redone each release (new hash). The durable fix is code signing (deferred).
See `WINDOWS.md` for the user-facing bypass + the web-exe fallback.
...
**Identity and trust.** Ed25519 accounts with an Argon2id-encrypted keystore and
a 24-word recovery phrase; safety-number verification; multi-device linking (QR
pair a phone or second desktop, with device certificates that keep MLS identity
intact) and a "they linked a new device" notice in the conversation.

**Networking.** libp2p transport over TCP and QUIC with Noise; mDNS, remembered
peers, DHT and opt-in public-DHT discovery; hole punching; circuit relay over the
rendezvous *and* over friends; a pinnable listen port for a fully direct
connection; a self-hostable rendezvous / relay / mailbox / gateway node.

**Messaging.** MLS-encrypted guilds and channels; replies, edits, deletes,
reactions, pins; Markdown with code blocks, lists and quotes; clickable
`@mentions`, plus `@role` and `#channel`, so a group too small to need
permissions still gets "@movie-night, 9pm?"; encrypted 5 MiB image attachments
and 25 MiB files, fetched out of band and cached; per-guild GIF packs and custom
emoji; GIF search proxied through your own rendezvous, which fetches the images
too, so no member's browser ever contacts the provider (§6.6); native link
previews and click-to-play YouTube embeds, both off by default; channel
categories; roles and permissions on a signed, replayable governance log; local
full-history search; Markdown export; offline history sync; encrypted-at-rest
storage.

**Direct messages.** A friends list and end-to-end-encrypted 1:1 DMs, with an
encrypted offline mailbox on the rendezvous (ciphertext-only deposits, optional
contentless push wakes) so DMs land even when the recipient is offline.
**Message requests**: a DM from someone you share no server with, have not
...
you are online — until you accept. Reciprocal typing indicators you can switch
off (read receipts need no switch: read state only ever travels to your own
devices).

**Voice and video.** WebRTC mesh with a speaking-ring participant panel, tuned
Opus, a noise gate, push-to-talk (bind any key; in-window only, because claiming
a key from the whole desktop means reaching outside the binary), camera video and
screen sharing with focus and theatre tiles, and per-device selection with a live
level meter and test tone. Optional TURN relay for IP-private calls — read §9.2
before deploying one.

**Meetings and guests.** Instant meeting rooms with a chosen lifetime; browser
guests with no install, chat plus audio, camera and screen share, scoped to one
channel of one meeting, with a lockable knock-to-enter door.

**Presence and polish.** Live presence, typing, and full profiles (avatar emoji,
accent colour, custom status, drag/paste + crop avatar editor); quick switcher
(Ctrl+K) and keyboard navigation; unread counts and per-server / per-channel
notification levels (all messages, only mentions, nothing) with a Do Not Disturb
that genuinely silences rather than decorating; synthesized voice and mention
sounds; system join/create notices; desktop notifications.

**Shipping.** Browser and native desktop front ends from one core; native mobile
apps (Capacitor Android and iOS shells over a gomobile core, with Android APKs in
releases); signed releases with in-app self-update, and peer-to-peer update
delivery when GitHub is unreachable.

**Next.** An optional SFU (forwarding still-encrypted frames) for larger voice
rooms; onion-routed metadata privacy; signed GIF-pack and emoji records to close
§12's asymmetry; sealing the peer cache; iOS distribution to match Android's
turnkey install.

**Known gaps** are enumerated in [§13](#13-threat-model--what-concord-defends-against)
...
42,000 lines. Every dependency is pure Go — no C toolchain — which is what makes
"one file, download and run" possible on every platform.
...
invariants; the guest framing contract (§14); and in-process multi-peer
...
Concord is free and open-source software, licensed under the **GNU Affero
