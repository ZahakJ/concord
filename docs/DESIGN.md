...
When no media relay is available, calls connect directly. The privacy page
leaves "hide my IP" as a preference — greying it out locked the switch, and
left it stuck on if it had already been flipped — and says in plain words that
a connected rendezvous is not a media relay. Creating a meeting, and the guest
page, warn the same way. Joining a call does not toast "no relay". If they
asked to hide their IP and TURN is missing, the app says so once. Nothing
silently pretends.
