...
type clientConfig struct {
	...
	// ed25519Cred is a Concord-added override (see WithEd25519SignatureKey):
	// a caller-supplied basic credential + deterministic Ed25519 signing key,
	// used so signing survives process restarts.
	ed25519Cred *credentials.CredentialWithKey
	logger      *slog.Logger
...
func NewClient(identity []byte, cs ciphersuite.CipherSuite, opts ...ClientOption) (*Client, error) {
	...
	if cfg.credentialWithKey != nil {
		...
		sigKey = cfg.sigKey
	} else if cfg.ed25519Cred != nil {
		// Concord: caller supplied a deterministic Ed25519 signing key.
		if cs.SignatureScheme() != ciphersuite.ED25519 {
			return nil, fmt.Errorf("WithEd25519SignatureKey requires an Ed25519 cipher suite")
		}
		if len(identity) == 0 {
			return nil, ErrEmptyIdentity
		}
		credWithKey = cfg.ed25519Cred
		sigKey = cfg.sigKey
	} else {
		if len(identity) == 0 {
			return nil, ErrEmptyIdentity
